...
bool Warrior_Lv(
   ...
   HTBasicFunc& arg_basic_func   // 基本函数集合
	...
	int warrior_lv = player[arg_player].cards_in_hand_[arg_card].level_;
    ...
    } else target = warrior_lv_choose_target(arg_player, arg_global_data);

    if (target == 0) return false; // 未找到目标直接返回

	player[arg_player].num_action_points_--;
	arg_global_data.attacking_player_ = arg_player; // 将攻击者设为 arg_player
	...
	if (sword == USE_SAINT_SWORD) { // 使用圣剑
		...
		int level = cards_abandoned.back().level_;

		CheckNun(target, arg_global_data);
		
		if (level != CARD_NO_LEVEL && level < warrior_lv) {
			player[arg_player].num_coins_++;
			arg_basic_func.pfn_gaming_interface_(arg_global_data);
			
			printf("丢弃的卡牌 Lv 小于勇者 Lv，");
		...
		// 结算
		if (target_card.level_ != CARD_NO_LEVEL && target_card.level_ < warrior_lv) {
			// 加钱
            player[arg_player].num_coins_++;
            arg_basic_func.pfn_gaming_interface_(arg_global_data);
            printf("攻击成功，");
...
int warrior_lv_choose_target(int arg_player, HTBasicData& arg_global_data) {
	sword = warrior_lv_choose_sword(arg_player, arg_global_data);
	...
	case USE_SAINT_SWORD:
		return saint_sword_targets[rand() % num_ordinary_targets];
	}
